...
model_name_or_path="scenario_cot_ratio_1.5B" 
tokenizer = AutoTokenizer.from_pretrained(model_name_or_path)
sampling_params = SamplingParams(temperature=0.1, top_p=0.1, repetition_penalty=1.05, max_tokens=4096) 
model = LLM(model=model_name_or_path,tensor_parallel_size=2, pipeline_parallel_size=1, dtype='float16',gpu_memory_utilization=0.7) 
...
def chunking_by_llm(content):
    system_prompt='''这是一个文档记忆提取任务，你是一名记忆提取方面的专家。首先，认真分析下面提供的内容，生成解决这个问题的逻辑流畅自洽且推理完整的思考过程。然后，利用对文档的全局理解，生成一个记忆提取大纲，再依据大纲为给定的文档生成一组对应数量的场景记忆。
...
save_path='mom/crud_cot_ratio_15B.json'
...
# CUDA_VISIBLE_DEVICES=4,5 nohup python mom_2.py >> mom/crud_cot_ratio_15B.log 2>&1 &
